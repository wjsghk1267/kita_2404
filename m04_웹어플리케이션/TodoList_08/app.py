...
def edit_task(task_id):
    ...
    return render_template(
        "edit_task.html",
        form=form,
        # task_id=task.id, (task에 포함되어 있음. 굳이 안써도됨.)
        csrf_token=csrf_token,
        task=task,  # 여기에 task 변수를 추가합니다.
        # task_title=task.title, (task에 포함되어 있음. 굳이 안써도됨.)
        # task_contents=task.contents, (동일)
        # task_input_date=task.input_date, (동일)
        # task_due_date=task_due_date,(동일)
        # completion_date=completion_date,  # completion_date 변수를 추가합니다.(동일)
